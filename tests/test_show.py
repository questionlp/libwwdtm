...
def test_id_exists(database_connection: mysql.connector.connect):
    ...
    response, response_code = show.id_exists(1091, database_connection)
...
def test_id_not_exists(database_connection: mysql.connector.connect):
    ...
    response, response_code = show.id_exists(-1, database_connection)
...
def test_date_exists(database_connection: mysql.connector.connect):
    ...
    response, response_code = show.date_exists(2006, 8, 19, database_connection)
    assert (response, response_code) == (True, ResponseCode.SUCCESS)
...
def test_date_not_exists(database_connection: mysql.connector.connect):
    ...
    response, response_code = show.date_exists(2006, 8, 18, database_connection)
    assert (response, response_code) == (False, ResponseCode.NOT_FOUND)

def test_retrieve_by_id(database_connection: mysql.connector.connect):
    """Testing response from show.retrieve_by_id"""
    show_details, response_code = show.retrieve_by_id(1091, database_connection)
...
def test_retrieve_by_invalid_id(database_connection: mysql.connector.connect):
    ...
    show_details, response_code = show.retrieve_by_id(-1, database_connection)
...
def test_retrieve_by_date(database_connection: mysql.connector.connect):
    ...
    show_details, response_code = show.retrieve_by_date(2006, 8, 19, database_connection)
    assert show_details is not None
    assert "host" in show_details
    assert "scorekeeper" in show_details
    assert "panelists" in show_details
...
def test_retrieve_by_invalid_date(database_connection: mysql.connector.connect):
    """Testing repsonse from show.retrieve_by_date with incorrect date"""
    show_details, response_code = show.retrieve_by_date(2006, 8, 18, database_connection)
    assert show_details is None
    assert response_code == ResponseCode.NOT_FOUND

def test_retrieve_by_date_string(database_connection: mysql.connector.connect):
    """Testing repsonse from show.retrieve_by_date_string"""
    show_details, response_code = show.retrieve_by_date_string('2006-08-19', database_connection)
    assert show_details is not None
    assert "host" in show_details
    assert "scorekeeper" in show_details
    assert "panelists" in show_details
...
def test_retrieve_by_invalid_date_string(database_connection: mysql.connector.connect):
    """Testing response from show.retrieve_by_date_string with invalud date string"""
    show_details, response_code = show.retrieve_by_date_string('2006-08-32', database_connection)
    assert show_details is None
    assert response_code == ResponseCode.BAD_REQUEST

def test_retrieve_by_year(database_connection: mysql.connector.connect):
    """Testing response from show.retrieve_by_year"""
    show_details, response_code = show.retrieve_by_year(2018, database_connection)
    assert show_details is not None
    assert response_code == ResponseCode.SUCCESS

def test_retrieve_by_year_month(database_connection: mysql.connector.connect):
    """Testing response from show.retrieve_by_year_month"""
    show_details, response_code = show.retrieve_by_year_month(2006, 8, database_connection)
    assert show_details is not None
    assert response_code == ResponseCode.SUCCESS

def test_retrieve_recent(database_connection: mysql.connector.connect):
    """Testing response from show.retreive_recent"""
    show_details, response_code = show.retrieve_recent(database_connection)
    assert show_details is not None
    assert response_code == ResponseCode.SUCCESS
